...
public class RecurentWithOut {
    ...
    public static void main(String[] args){
        ...
        RecurentStart(n, arrOfLn, status, arrayOfLists);
    ...
    */
    ...
    private static int RecurentStart(int n, int[] numBefore, int[] status, int[][] arrayOfLists){
        ...
        int fullTime = CalcMax(resTime);
        System.out.println(fullTime);

        // ArrayList<ArrayList<Integer>> listOfArrayLists = new ArrayList<>();

        for(int i = 1; i <= fullTime; i++){
            ArrayList<Integer> localGroup = new ArrayList<Integer>();
            for(int j = 0; j < n; j++){
                if(resTime[j] == i){
                    localGroup.add(j+1);
                }
            }
            System.out.print(localGroup.size() + " ");
            for(int j = 0; j < localGroup.size(); j++){
                System.out.print(localGroup.get(j));
                if(j < localGroup.size() - 1){ System.out.print(" "); }
            }
            if(i < fullTime){ System.out.print("\n"); }
        }
        return fullTime;
    }
